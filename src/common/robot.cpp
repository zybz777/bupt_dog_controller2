...
void Robot::init() {
    ...
    for (int i = 0; i < body_inertial_mat.size(); ++i) {
        ...
        body_com[i] = _robot_model->inertias[_robot_model->getJointId(body_joint_names[i])].lever() + _robot_data->oMi[_robot_model->getJointId(body_joint_names[i])].translation();
